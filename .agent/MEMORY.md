...
**FE-021 accepted 2026-09-24, uncommitted until the owner asks.** 1645 tests / 23 files. 5 build rounds, 12 review findings, all fixed. Lead browser check at 1280×800: the footer rows fit, and Fish ×2 from `default` gives 400.000 / total 760.725. **Follow-up (not filed):** the order list doesn't scroll to a newly added line, which lands below the fold. `builder23` and `f5a-reviewer` are closed, with panes `w2:p1F` and `w2:p1G`.
