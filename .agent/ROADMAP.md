...
| F2e | **Four corrections to committed work.** (1) acting controls become `<button>` across F2a and F2b; (2) `role="alert"` on both PIN pads' failure notices; (3) F2c's sheets replace history rather than pushing, per SITEMAP §1; (4) **the panel's void paths carry the actual line and order** — today every fired row opens one fixture sheet, so tapping Soda offers to void the Burger, which behind a real command is a cancellation ticket for work nobody asked to cancel (`B-16`). (4) needs (1), which is why it is one job | **Next, and no longer tidying.** **Ruled 2026-09-17** after two slices reported the same unverifiable focused-and-pressed state: Space does not activate a link. Tiles, categories, line bodies and the remove control act on the order and are buttons; anchors are for leaving the screen. A refactor of committed work, so its own task | unassigned |
