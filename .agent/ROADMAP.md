...
Previously 2026-09-23. The owner merged `agent/phase-0-foundations` into
...
| A10 | [DESIGN-006](../../restaurant-pos-design/.agent/tasks/DESIGN-006-settlement-corrections.md): POS-04's five artifact defects and six lead rulings to confirm. **The owner ruled it goes before F4** | **Delivered 2026-09-24**, uncommitted in the design worktree: 34 states, and the lead measured AC1 in a browser. The design review asked for changes (3 P2 + 1 P3, all accepted). **Done 2026-09-24**: round 2 was verified by the lead in a browser and committed on the design branch as `c43fd08` and `35b66f9`, not pushed. **[FE-020](tasks/FE-020-settlement-design-corrections.md) brought the code up to it, done 2026-09-24** at 1349 tests and walked in a browser | `designer3` (codex `gpt-6-astra`) |
...
| **Render the `cancel`, `reauth` and `leaselost` modals inside `.pos-device`** | FE-020 moved takeover's modal inside the device frame because it centred on the browser viewport and ran off at 958px tall. The other three still sit outside and fit only because they are short. Same fix, no design question |
| ~~Collapse `OrderScreen`'s two stores into one path~~ | **Done in FE-019 (`34cb966`).** F3a lifted the store above both screens but left `OrderScreen` building its own `useOrderStore` behind `suppliedStore ?? localStore`, so every URL-driven mutation had two listeners and only the discarded one heard it. `builder19` found it; F3c patches it narrowly (`onLocationChange` on every navigate). The two paths remain. Take it up with the F3a–F3d review |
...
| F4-design | [DESIGN-008](../../restaurant-pos-design/.agent/tasks/DESIGN-008-floor-and-incident-corrections.md): POS-02 floor in Frost (ten states), plus three POS-07 corrections (per-incident reprint, a clear control, two missing tokens). A design review follows | **Delivered 2026-09-25** after 2 rounds (the lead ruled no confirm, and fixed its own FR-C8 error); review requested changes (2 P2, both shared-destination defects), round 3 fixed them, lead-verified in Chrome. **Accepted 2026-09-25**, uncommitted | `designer6`, `design-reviewer4`, `designer7` (all closed) |
| F4b | [FE-026](tasks/FE-026-floor-and-order-book.md): POS-02 floor + an order book (one order per table, active id in the store not the URL, payment lock per order, `← Floor` on the order bar) | **Done 2026-09-25**, accepted after 2 rounds (2203 tests / 30 files, lead re-ran and walked it in Chrome: per-table orders, derived tiles, per-order payment). Two lead task errors (141.750, `Order · T<n>`) and one in-slice money defect (drafts crossing orders) fixed in round 2. P3: a reopened book-only order lands on `?state=default` | `builder28` (closed) |
| F4c | [FE-027](tasks/FE-027-close-closes.md): Close actually closes — pure `closeOrder`, drafts become tenders, quick sale fires at close, the book keeps closed orders, Close lands on the floor (lead ruling; artifact says POS-06 — DESIGN-009 to settle) | **Done 2026-09-25** (2224 tests / 32 files, lead re-ran and walked it: close once, floor, table free, Back cannot reach a closed order). 2 approved existing-test edits. P3s: `fireOrder` called as `table` for a quick sale; silent refusal for a quick sale with an 86'd line | `builder29` (closed) |
| F4d | [FE-028](tasks/FE-028-incident-recovery.md): per-card checked Clear (kitchen, not audited — owner 2026-09-25), Dismiss (receipt), DESIGN-008's copy and fixture states; the four pinned tests named in advance | **Done 2026-09-25** (2253 tests / 32 files; lead re-ran and walked every recovery path in Chrome). P3: *Nothing outstanding* drawn twice when all cleared | `builder30` (closed) |
| F4-design-2 | DESIGN-009: POS-05 closed orders + POS-06 closed order detail (refund sheet M-5, manager approval) | Not written; after DESIGN-008 | unassigned |
